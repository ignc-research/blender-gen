...
class BlenderGen:
    ...
    def save_coco_label(self, images, annotations, Kdict):
        ...
        filename = "DATASET/" + self.cfg.out_folder + "/annotations/instances_default.json"
    ...
    def import_obj_object(self, filepath, distractor=False):
        ...
        if (distractor == True and texture_path):  # import original distractor png texture
            ...
            texture.image = bpy.data.images[os.path.split(texture_path)[1]]
                ...
                len(self.cfg.distractor_texture_path) > 0):  # distractor with random texture
        ...
        if (len(self.cfg.object_texture_path) > 0 and
    ...
    def setup_light(self, scene, light_number=1, random_color=None):
        ...
        for i in range(light_number):
            ...
            light_data.energy = random.uniform(
                self.cfg.light_energymin, self.cfg.light_energymax)  # random energy in Watt
    ...
    def scene_cfg(self, camera, i):
        ...
        if (not self.cfg.use_environment_maps):
            light_nr = random.randint(
                self.cfg.light_number_min,
                self.cfg.light_number_max)  # sample number n of Point Lights
            self.setup_light(scene, light_number=light_nr, random_color=self.cfg.random_color)
        ...
        if (self.cfg.use_environment_maps):
            ...
            bpy.data.worlds['World'].node_tree.nodes['Emission'].inputs[
                1].default_value = random.uniform(self.cfg.emission_min,
                                                self.cfg.emission_max)
        ...
        if (len(self.cfg.distractor_texture_path) > 0):
            for distractor in mesh_list_distractors:
                ...
                texture_path = texture_list[random.randint(0,
                                                        len(texture_list) - 1)]
                bpy.data.images.load(self.cfg.distractor_texture_path + '/' +
                                    texture_path)
        ...
        for j in range(n):
            ...
            dis_obj.location.x = random.uniform(self.cfg.obj_location_xmin,
                                                self.cfg.obj_location_xmax)
            dis_obj.location.y = random.uniform(self.cfg.obj_location_ymin,
                                                self.cfg.obj_location_ymax)
            dis_obj.location.z = random.uniform(self.cfg.obj_location_zmin,
                                                self.cfg.obj_location_zmax)
            rot_angle1 = random.uniform(self.cfg.cam_rotation_min, self.cfg.cam_rotation_max)
            rot_angle2 = random.uniform(self.cfg.cam_rotation_min, self.cfg.cam_rotation_max)
            rot_angle3 = random.uniform(self.cfg.cam_rotation_min, self.cfg.cam_rotation_max)
        ...
        if (self.cfg.random_metallic_value):
            if (random.random() >= 0.5):
                ...
                mat.node_tree.nodes["Principled BSDF"].inputs[
                    'Metallic'].default_value = self.cfg._metallic[x - 1]
        ...
        if (self.cfg.random_roughness_value):
            if (random.random() >= 0.5):
                ...
                mat.node_tree.nodes["Principled BSDF"].inputs[
                    'Roughness'].default_value = self.cfg._roughness[x - 1]
        ...
        while (repeat):
            ...
            camera = self.place_camera(camera,
                                radius=random.uniform(self.cfg.cam_rmin, self.cfg.cam_rmax),
                                inclination=random.uniform(
                                    self.cfg.cam_incmin, self.cfg.cam_incmax),
                                azimuth=random.uniform(self.cfg.cam_azimin,
                                                        self.cfg.cam_azimax))

            empty_obj = bpy.data.objects["empty"]

            # random object pose
            obj.location.x = random.uniform(self.cfg.obj_location_xmin,
                                            self.cfg.obj_location_xmax)  # x_o
            obj.location.y = random.uniform(self.cfg.obj_location_ymin,
                                            self.cfg.obj_location_ymax)  # y_o
            obj.location.z = random.uniform(self.cfg.obj_location_zmin,
                                            self.cfg.obj_location_zmax)  # z_o

            rot_angle1 = random.uniform(self.cfg.cam_rotation_min,
                                        self.cfg.cam_rotation_max)  #  alpha 1
            rot_angle2 = random.uniform(self.cfg.cam_rotation_min,
                                        self.cfg.cam_rotation_max)  #  alpha 2
            rot_angle3 = random.uniform(self.cfg.cam_rotation_min,
                                        self.cfg.cam_rotation_max)  #  alpha 3
            ...
            center = self.project_by_object_utils(camera,
            ...
            for corner in corners:
                ...
                p = self.project_by_object_utils(camera, p)  # world space to image space
                labels.append(p[0])
                labels.append(p[1])
                ...
                kps.append([p[0] * self.cfg.resolution_x, p[1] * self.cfg.resolution_y, v])

                # filter out objects outside of the image view
                if (p[0] < -self.cfg.max_boundingbox or p[0] >
                    (1 + self.cfg.max_boundingbox) or p[1] < -self.cfg.max_boundingbox or
                        p[1] > (1 + self.cfg.max_boundingbox)):
                    repeat = True
            ...
            if (self.cfg.use_fps_keypoints == False):
                kps.insert(
                    0,
                    [self.cfg.resolution_x * center[0], self.cfg.resolution_y * center[1], 2
                    ])  # center is the 1st keypoint
            ...
            if (not repeat):
                ...
                annotation = {
                    ...
                    "image_id": i,
                    "bbox": [
                        min_x * self.cfg.resolution_x, min_y * self.cfg.resolution_y,
                        x_range * self.cfg.resolution_x, y_range * self.cfg.resolution_y
                    ],
                    "category_id": 1,
                    "segmentation": [],
                    "iscrowd": 0,
                    "area": x_range * self.cfg.resolution_x * y_range * self.cfg.resolution_y,
                    "keypoints": kps,
    ...
    def render_cfg(self):
        ...
        if (self.cfg.use_cycles):
            ...
            bpy.context.scene.cycles.use_adaptive_sampling = self.cfg.use_adaptive_sampling
    ...
    def render(self, camera, depth_file_output):
        ...
        for i in range(self.cfg.numberOfRenders):
            ...
            bpy.context.scene.render.filepath = './DATASET/' + self.cfg.out_folder + '/images/{:06}.jpg'.format(
                i)
    ...
    def run(self):
        ...
        shutil.copy2('config.py',
                    'DATASET/' + self.cfg.out_folder)  # save config.py file
        self.save_coco_label(images, 
                    annotations,
