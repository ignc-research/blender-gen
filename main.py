...
def setup_light(scene):
    ...
    #x,y,z = get_sphere_coordinates(radius=3, inclination=random.uniform(0,math.pi), azimuth=random.uniform(0, 2*math.pi))
    x = random.uniform(cfg.obj_xmin, cfg.obj_xmax)
    y = random.uniform(cfg.obj_ymin, cfg.obj_ymax)
    z = random.uniform(cfg.obj_zmin, cfg.obj_zmax)
    lamp_data = bpy.data.lights.new(name='Light', type='POINT')
    lamp_data.energy = np.random.uniform(60, 1000)  # random energy in Watt
...
def add_shader_on_world():
    ...
    bpy.data.worlds['World'].node_tree.links.new(env_node.outputs['Color'], world_node.inputs['Surface'])
...
def scene_cfg(camera, i):
    ...
    while (repeat):
        ...
        #camera=place_camera(camera, radius=random.uniform(cfg.camera_min_r, cfg.camera_max_r), inclination=random.uniform(0,math.pi), azimuth=random.uniform(0, 2*math.pi))
        ...
        if(not repeat):
            f = open("./DATASET/phone/labels/{:06}.txt".format(i),'ab')
...
def render(camera, depth_file_output, test_flag):
    ...
    for i in range(cfg.numberOfRenders):
        bpy.context.scene.render.filepath = './DATASET/phone/images/{:06}.png'.format(i)
        bg_img = scene_cfg(camera, i)
        if (cfg.output_depth):
...
def main():
    ...
    render(camera, depth_file_output, args.test)  # render loop
    K, RT = get_camera_KRT(bpy.data.objects['Camera'])
    save_camera_matrix(K)  # save Camera Matrix to K.txt
    ...
    if args.test:
        import cv2
        image = cv2.imread('./DATASET/phone/images/{:06}.png'.format(cfg.numberOfRenders-1))
    
        # load data 
        BB = np.loadtxt("model_bounding_box.txt")
        label = np.loadtxt("./DATASET/phone/labels/{:06}.txt".format(cfg.numberOfRenders-1))
        ...
        K = np.loadtxt("K.txt")
        x = (round(center[0,0]), round(center[0,1]))
        cv2.circle(image, x, 1, (255, 0, 0), 3)  # center point in blue
        for i in range(8):
        ...
        axis1 = np.float32([[100,0,0], [0,100,0], [0,0,-100]]).reshape(-1,3)
